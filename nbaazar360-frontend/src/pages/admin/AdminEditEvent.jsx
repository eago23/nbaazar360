...
function AdminEditEvent() {
    ...
    try {
      ...
      if (event) {
        ...
        setFormData({
          ...
          start_date: formatDate(event.start_date || event.event_date),
      ...
      setError('Gabim gjate ngarkimit te ngjarjes')
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await adminEventsAPI.update(id, eventData)
      navigate('/admin/permbajtje')
    } catch (error) {
      console.error('Error updating event:', error)
      setError(error.response?.data?.error || error.response?.data?.message || 'Gabim gjate perditesimit te ngjarjes')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center space-x-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-secondary">Ndrysho Ngjarjen</h1>
      </div>
